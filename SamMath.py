...
def realRoots(inlist):
	...
	#and q is the trailing coefficient.
		...
		outlist = []
		p = abs(inlist[-1])	#p is the leading coefficient (ignoring sign)
		q = abs(inlist[0])	#q is the trailing coefficient (ignoring sign)
		
		#find all factors of p and q
		pfactors = list(factors(p))
		qfactors = list(factors(q))

		#build list of p/q factors
